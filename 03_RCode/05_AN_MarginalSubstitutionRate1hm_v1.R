...
land.MSR.estimation <- function(land_value, inc_value, input_land_model, 
  ...
  income_change <- c()
  for(single_g_Inc_delta in g_Inc_delta){
    income_change_single <- try(calculate_root(single_g_Inc_delta, input_inc_model), silent = TRUE)
    if(!inherits(income_change_single, "try-error")){
      income_change_single <- calculate_root(single_g_Inc_delta, input_inc_model)
    } else {
...
calculate_root <- function(est_MH_inc, input_inc_model = input_inc_model){
  coef <- coefficients(input_inc_model)
  root <- uniroot(f, c(-1, 1), coef = coef, b = est_MH_inc, tol = 0.0001)
  return(root$root)
}
